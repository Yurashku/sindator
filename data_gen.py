...
class DataGenerator:
    ...
    def __init__(self, n_samples=2000, distributions=None, 
                 time_correlations=None, effect_size=5.0, seed=None):
        ...
        self.time_correlations = time_correlations or {
            'X1': 0.7,
            'X2': 0.6,
            'y0': 0.8
        }
    ...
    def _generate_correlated_pair(self, dist_type, params, rho):
        ...
        if dist_type == 'normal':
            cov = [[params['std']**2, rho*params['std']**2],
                  [rho*params['std']**2, params['std']**2]]
            return np.random.multivariate_normal(
                [params['mean'], params['mean']], cov, self.n_samples).T
    ...
    def generate(self):
        ...
        if y_params['type'] == 'normal':
            ...
            lag2, _ = self._generate_correlated_pair(
                y_params['type'],
                y_params,
                y_rho
            )
        ...
        data['y1'] = data['y0'] + self.effect_size + 1.5*data['U'] + np.random.normal(0, 2, self.n_samples)
